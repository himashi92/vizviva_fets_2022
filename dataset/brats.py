...
from config import get_brats_folder, get_brats_test_folder
...
class Brats(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if _patient["seg"] is not None and self.training:
...
def get_test_datasets(on="train", normalisation="minmax"):
    base_folder = pathlib.Path(get_brats_test_folder()).resolve()
    print(base_folder)
    assert base_folder.exists()
    patients_dir = sorted([x for x in base_folder.iterdir() if x.is_dir()])
