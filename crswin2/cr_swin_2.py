...
class LePEAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        ### Window2Img
        x = windows2img(x, self.H_sp, self.W_sp, 32, H, W).view(B, -1, C)  # B H' W' C
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, block_num):
        ...
        B, C, D, H, W = x.shape
        window_size, shift_size = get_window_size((D, H, W), self.window_size, self.shift_size)
        x1 = rearrange(x, 'b c d h w -> b d h w c')
        x2 = rearrange(x, 'b c d h w -> b d h w c')
        Dp = int(np.ceil(D / window_size[0])) * window_size[0]
        Hp = int(np.ceil(H / window_size[1])) * window_size[1]
        Wp = int(np.ceil(W / window_size[2])) * window_size[2]
        attn_mask = compute_mask(Dp, Hp, Wp, window_size, shift_size, x.device)
...
class SwinTransformerSys3D(nn.Module):
    ...
    def __init__(self, pretrained=None,
        ...
        print(
            "SwinTransformerSys3D expand initial----depths:{};depths_decoder:{};drop_path_rate:{};num_classes:{}".format(
                depths,
